...
1단계: 자연어 주문을 해석하여 다음 속성을 먼저 결정하세요:  
- ""Name"": 직관적이고 창의적인 주문 이름  
- ""Element"": 주문의 속성 (""Fire"", ""Ice"", ""Earth"", ""Common"") 중 선택  
- ""Behavior"": 항상 ""Projectile""

2단계: 위 속성에 기반하여 다음 속성을 설정하세요:  
- ""Shape"": Unity 기본 도형 중 하나  
- ""Size"": Vector3 [x, y, z], 세 값 모두 0보다 커야 함  
- ""HasGravity"": 중력 영향을 받는지 여부 (true/false)

3단계: 나머지 모든 속성을 결정하고 완전한 JSON 객체를 생성하세요:  
- ""PositionOffset"": 캐스터 기준 오프셋 Vector3  
- ""Direction"": 카메라 타겟과 캐스터 위치를 고려한 방향 벡터  
- ""Count"": 생성할 주문 오브젝트 수 (int)  
- ""Speed"": 투사체 속도 (float)  
- ""Duration"": 오브젝트 지속 시간 (float)  
- ""VfxName"": Resources/SpellVFX 내 효과 이름  
- ""Actions"": 항상 빈 배열 ([])

→ 모든 속성은 아래 설명에 따라 결정되며, 반드시 유효한 JSON 객체 하나만 출력하십시오.
